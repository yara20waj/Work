...
if(isset($_SESSION['user_id'])){
	...
	$chg_pwd=mysqli_query($con,"SELECT * from tblemployees WHERE id='$user_id'");
	$chg_pwd1=mysqli_fetch_array($chg_pwd);
	$data_pwd=$chg_pwd1['Password'];
	if($data_pwd==$old_password){
	if($new_password==$con_password){
	$update_pwd=mysqli_query($con,"UPDATE tblemployees set Password='$new_password' where id='$user_id'");
                    ...
                    <div class="box">
                        <form class="col s12" id="chngpwd" method="post" name="chngpwd">
                            ...
                            <div class="input-field col s12">
                                <input autocomplete="off" class="validate" id="password" name="password" required=""
                                    type="password"> <label for="password">Enter Old Password</label>
                            </div>
                            <div class="input-field col s12">
                                <input autocomplete="off" class="validate" id="password" name="newpassword" required=""
                                    type="password"> <label for="password">Enter New Password</label>
                            </div>
                            <div class="input-field col s12">
                                <input autocomplete="off" class="validate" id="password" name="confirmpassword"
                                    required="" type="password"> <label for="password">Enter Confirm Password</label>
                            </div>
                            <div class="input-field col s12">
                                <button class="change" name="change" onclick="return valid();"
